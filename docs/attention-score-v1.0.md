...
3. fixture の competition canonical ID が configured overseas scope に含まれ、`competitions.type` が `League` または `Cup`。friendly / preseason は設定で除外し `not_applicable`。
...
| 出場状況 | `fixture_player_records.appearance_state`、`minutes` | 必須 |
...
| 関与した追跡選手数 | `tracking_periods` × `fixture_player_records` | 必須 |

Core の同じ facts を tracking 側へ複製しない。計算時に公開 fixture revision と fixture scope の player record を結合し、`inputs_json` には使用値・presence・canonical IDだけを保存する。
...
`comeback_bonus` は時系列 event が `presence: present` の場合だけ判定する。score parts / events が未取得、競合、順序不明なら0にせず、base score 全体を未算出にする。延長・PKは `status_short` と score parts の双方が一致するときだけ加点する。
...
v1.0 は公式 `League` と `Cup` をどちらも係数 `1.00` とする。provider の2値だけから大会の格を推測しない。friendly / preseason は workflow policy と同じくランキング対象外とする。将来、大会別係数を導入する場合は Git 管理の明示設定と根拠を追加し、`attention_version` を上げる。
...
入力状態と採用 source は `inputs_json` に残し、`source_hash` は canonical JSON の SHA-256 とする。
...
中立ランキングの表示閾値は `displayed_score >= 20.00` とする。閾値未満になった fixture は一覧から自然に消える。
...
personal_displayed_score = displayed_score × follow_multiplier
```

個人係数は表示順と表示閾値だけに使い、`base_score`、`source_hash`、サーバの中立順位を変更しない。
