...
recordを持たないsnapshot playerも検証から除外しない。`trackedPlayerIdentities` gateが、未解決identity、tracking属性、legacy membershipのラベル・期間・status・change type・verification・source hash、およびlegacy season aggregateを固定snapshotと完全照合する。legacy season aggregateは`seasonStats`、`allCompetitionsStats`、`competitionStats`、`clubStats`、`clubCompetitionStats`、`_aggregateBaselines`に加え、`statsScope`、`statsStatus`、`statsAsOf`、`statsTrackingState`、`_initialStats`、`_initialClub`、`_initialLeague`、`_initialStatsCaptured`、`_initialStatsUpdated`を欠落なく保持する。reportの`preservedAggregateFields`はこの完全な契約を列挙する。match由来provider evidenceがない段階でCore team / competition identityを推測した正規化scope rowは作らず、固定snapshotの移行中は同一playerのaggregate row集合が対象product seasonのlegacy `season` row 1件だけであることを検証する。別product seasonの行もこのPhase 2契約では余剰scopeとして拒否する。readinessの期待値はimporter helperを使わず固定snapshotから独立に構築する。match由来provider evidenceがない正常状態は`no_match_evidence`として明示し、欠落・余剰scope・改変・根拠のないresolved化は`failed`としてgateを閉じる。
