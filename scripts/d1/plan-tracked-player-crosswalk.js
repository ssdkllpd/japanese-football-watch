...
const { buildTrackedPlayerCrosswalkPlan } = require('./tracked-player-crosswalk-plan');
...
function main(argv = process.argv.slice(2)) {
  ...
  if (!args['--snapshot'] || !args['--coverage'] || !args['--database'] || !args['--output']) {
    throw new Error('Usage: node scripts/d1/plan-tracked-player-crosswalk.js --snapshot FILE --coverage FILE --database FILE --output FILE');
  }
  ...
  try {
    ...
    fs.writeFileSync(args['--output'], `${stableStringify(plan)}\n`);
